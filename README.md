...
| :---        | :----   | :--- |
| Connor Buckley | Blackjack | I implemented the Blackjack backend, including the full game model and core round logic. I built the deck, card, participant, player, dealer, and game state classes that drive betting, dealing, turn flow, and result handling. I also added save and load support for Blackjack via Jackson so a game state can be serialized and restored later. My work covered everything in the blackjack model package and provided the logic used by the frontend. Check `src/main/java/edu/sjsu/cs151/blackjack/blackjack.md` to see how frontend connects to backend.|
